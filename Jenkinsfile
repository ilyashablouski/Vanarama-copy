        ...
        stage("4. Production Build & push") {
            ...
            steps {
                ...
                script {
                    ...
                    withCredentials([string(credentialsId: 'npm_token', variable: 'NPM_TOKEN')]) {
                    sh """
                      #API_KEY="$(aws ssm get-parameter --name "/${ENV}/${STACK}/${APP}/API_KEY" --region $REGION --with-decryption | jq -r ".Parameter.Value")"
                      #API_URL="$(aws ssm get-parameter --name "/${ENV}/${STACK}/${APP}/API_URL" --region $REGION --with-decryption | jq -r ".Parameter.Value")"
