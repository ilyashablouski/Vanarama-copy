...
describe('<FleetLandingPage />', () => {
    ...
    it('should render testimonial section correctly', async () => {
        ...
        // ASSERT
        await waitFor(() => {
            expect(screen.getByText(`I can't really compare you with any of the other companies out there as I've not used them. I guess that speaks for itself!`)).toBeInTheDocument();
        });
        expect(screen.getByText(`Wherever You Are, However You Want To Work, We're There For You`)).toBeVisible();
        expect(screen.getByTestId('fleet_testimonial-section_request-button')).toHaveTextContent("I'd Like A Callback");
    });
    ...
    it('should render side media sections according to layout type', async () => {
        ...
        // ASSERT
        await waitFor(() => {
            expect(screen.getByText(`What's In It For My Business?`)).toBeInTheDocument();
        });
        expect(screen.getByText(`What's In It For My Business?`).parentElement?.parentElement).toHaveClass('row:featured-left');
        expect(screen.getByText(`What's In It For Me?`).parentElement?.parentElement).toHaveClass('row:featured-right');
        expect(screen.getByText(`What's In It For The Drivers?`).parentElement?.parentElement).toHaveClass('row:featured-left');
    });
