...
describe(
  ...
  () => {
    ...
    beforeEach(() => {
      ...
      cy.get(
        'ul[data-uitestid="header_secondary-menu_CARS_menu-tertiary_Cars By Type"]>li:nth-child(3)',
      ).click();
    });

    it('should show relevant results', () => {
      cy.get('.card.product').should('contain.text', 'Convertible');
    });
    ...
    it('filtering results by body type and fuel type', () => {
      ...
      cy.get(`span[data-uitestid="${dataUiTestId}_span_Fuel Type"]`)
        ...
        .click();
      cy.get(`span[data-uitestid="${dataUiTestId}_span_Petrol"]`).click();
      cy.get('@FuelTypeAccordion').click();
      cy.location().should(location => {
        expect(location.search).to.eq(`?fuelTypes=Petrol&pricePerMonth=0|350`);
      });
    });
